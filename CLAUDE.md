...
App Router under `src/app/[locale]/` with `[locale]` ∈ `{en, es}`, but **English (the default) is served prefix-less**: the canonical URLs are `/` + `/app` (English) and `/es` + `/es/app` (Spanish). `src/middleware.ts` makes this work: it rewrites clean paths internally to `/en/*` (URL stays clean), permanently (308) redirects legacy `/en/*` URLs to the clean form, and lets `/es/*` through. (Next.js 16 renamed this convention to `proxy`, but `proxy.ts` is forced onto the Node.js runtime, which OpenNext/Cloudflare does not support — only an Edge `middleware.ts` deploys. Next will emit a deprecation warning for the `middleware` filename; ignore it until OpenNext supports Node proxy.) Locale preference is `NEXT_LOCALE` cookie → `Accept-Language` → `en`; a Spanish preference 307-redirects clean paths to `/es`. Build locale-aware URLs with `localeHref`/`localePrefix` from `src/lib/locale.ts` (never hardcode `/en`). The app shell is `/[locale]/app`; `/[locale]` is the marketing landing page.
