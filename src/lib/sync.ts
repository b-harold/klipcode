...
// Notes sync plaintext for now (no `crypto_version` column on the notes table);
// see CloudNoteRow. Extending encryption to notes is a pending follow-up.
function mapNoteToCloud(note: NoteRecord, userId: string): CloudNoteRow {
  return {
    id: note.id,
    owner_id: userId,
...
async function markNoteAsSynced(
  note: NoteRecord,
  userId: string,
  syncedAt: string
): Promise<boolean> {
...
export async function syncDirtyWorkspace(userId: string): Promise<SyncResult> {
  ...
  if (!supabase) {
    return { syncedFolderIds: [], syncedSnippetIds: [], syncedNoteIds: [], localSnippetIds: [] };
  }
  ...
  // Notes have no inter-note dependency either; one plaintext batched upsert.
  if (dirtyWorkspace.notes.length > 0) {
    const { error } = await supabase
      .from("notes")
      .upsert(
        dirtyWorkspace.notes.map((note) => mapNoteToCloud(note, userId)),
        { onConflict: "id" }
      );
    ...
    for (const note of dirtyWorkspace.notes) {
      const marked = await markNoteAsSynced(note, userId, syncedAt);
      if (marked) syncedNoteIds.push(note.id);
    }
  }
...
export async function fetchCloudWorkspace(userId: string) {
  ...
  const [
    ...
    supabase
      .from("snippets")
      .select(
        "id, owner_id, folder_id, title, code, language, source_url, is_pinned_aside, is_pinned_home, created_at, updated_at, deleted_at, crypto_version"
      )
      .eq("owner_id", userId),
    supabase
      .from("notes")
  ...
  const noteRows = notes as CloudNoteRow[];
  ...
  for (const noteRow of noteRows) {
    ...
    const currentNote = localNoteMap.get(incomingNote.id);

    if (currentNote?.dirty && currentNote.updatedAt >= incomingNote.updatedAt) {
      continue;
    }

    notesToPut.push(incomingNote);
  }
  ...
  await reconcileDeletions(
    ...
    new Set((notes as CloudNoteRow[]).map((note) => note.id)),
    localFolders,
    localSnippets,
    localNotes
  );
...
async function reconcileDeletions(
  ...
  cloudNoteIds: Set<string>,
  localFoldersSnapshot?: FolderRecord[],
  localSnippetsSnapshot?: SnippetRecord[],
  localNotesSnapshot?: NoteRecord[]
) {
  const [allFolders, allSnippets, allNotes] =
    localFoldersSnapshot && localSnippetsSnapshot && localNotesSnapshot
      ? [localFoldersSnapshot, localSnippetsSnapshot, localNotesSnapshot]
      : await Promise.all([db.folders.toArray(), db.snippets.toArray(), db.notes.toArray()]);
  ...
  const localNotes = allNotes.filter((note) => note.ownerId === userId);
  ...
  const notesToDelete = localNotes
    .filter((note) => !note.dirty && note.lastSyncedAt !== null && !cloudNoteIds.has(note.id))
    .map((note) => note.id);
...
async function accountHasCloudContent(userId: string): Promise<boolean> {
  ...
  try {
    const [{ data: folderRows }, { data: snippetRows }, { data: noteRows }] = await Promise.all([
      supabase.from("folders").select("id").eq("owner_id", userId).limit(1),
      supabase.from("snippets").select("id").eq("owner_id", userId).limit(1),
      supabase.from("notes").select("id").eq("owner_id", userId).limit(1),
    ]);

    return (
      (folderRows?.length ?? 0) > 0 ||
      (snippetRows?.length ?? 0) > 0 ||
      (noteRows?.length ?? 0) > 0
    );
  } catch {
    return false;
...
async function claimAnonymousRecords(userId: string): Promise<void> {
  ...
  let anonymousNotes = notes.filter((note) => note.ownerId === null);

  if (
    anonymousFolders.length === 0 &&
    anonymousSnippets.length === 0 &&
    anonymousNotes.length === 0
  ) {
    return;
  }

  const isPristineSeed = (record: FolderRecord | SnippetRecord | NoteRecord) =>
    !record.dirty && record.lastSyncedAt === null;

  const hasPristineSeed =
    anonymousFolders.some(isPristineSeed) ||
    anonymousSnippets.some(isPristineSeed) ||
    anonymousNotes.some(isPristineSeed);
  ...
  if (hasPristineSeed && (await accountHasCloudContent(userId))) {
    ...
    const seedNoteIds = new Set(
      anonymousNotes.filter(isPristineSeed).map((note) => note.id)
    );
    ...
    const remainingNotes = notes.filter((note) => !seedNoteIds.has(note.id));
    ...
    while (changed) {
      ...
      for (const folder of remainingFolders) {
        ...
        const hasChild =
          ...
          remainingNotes.some((note) => note.folderId === folder.id);
        if (!hasChild) {
    ...
    if (seedNoteIds.size > 0) {
      await db.notes.bulkDelete([...seedNoteIds]);
    }
    ...
    anonymousNotes = anonymousNotes.filter((note) => !seedNoteIds.has(note.id));
  }
  ...
  if (anonymousNotes.length > 0) {
    await db.notes.bulkPut(
      anonymousNotes.map((note) => ({ ...note, ownerId: userId, dirty: true }))
    );
  }
}
