...
- [ ] **Language choice isn't persisted.** The proxy (`src/proxy.ts`) redirects `/` purely by `Accept-Language`; a user who manually switches to `/es` gets bounced back to English on their next visit. Set a `NEXT_LOCALE` cookie on manual switch and prefer it in the proxy.
